...
struct SingleSensorWithGraphProvider: IntentTimelineProvider {
    ...
    func placeholder(in context: Context) -> SingleSensorWithGraphEntry {
        SingleSensorWithGraphEntry(date: Date(),device_id: "Placeholder", configuration: SingleSensorIntent(), timeSpan: .day, sensor: nil,dataDay: [], dataWeek:[], dataMonth:[])
    }
    ...
    func getTimeline(for configuration: SingleSensorIntent, in context: Context, completion: @escaping (Timeline<SingleSensorWithGraphEntry>) -> ()) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + seconds) {
            let entry = SingleSensorWithGraphEntry(date: Date(), device_id: selectableSensor?.identifier ?? "",  configuration: configuration, timeSpan: configuration.timeSpan, sensor: singleSensorVM.sensor, dataDay: hourlyAggregVM.dataDay, dataWeek: weeklyAggregVM.dataWeek, dataMonth: monthlyAggregVM.dataMonth)
